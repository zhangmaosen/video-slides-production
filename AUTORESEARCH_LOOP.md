...
### 输入
1. **参考图元数据** - 必须读取 `assets/ref_meta.json`，了解每张参考图的用途
2. **参考图** - 必须读取 `assets/` 目录下的参考图，分析特征
3. **slides_content.json** - 当前 slide 的内容
4. **CHANGELOG.md** - 迭代历史，了解之前版本的问题
5. **二郎神反馈** - 上一版本的扣分点和优化建议
6. **Rex 指令** - 告知女娲使用哪个版本作为基础（通常是最高分版本）
...
## 第二步：读取项目文件
1. 参考图：{project_path}/assets/（分析特征）
2. slides_content.json（当前 slide 内容）
3. CHANGELOG.md（迭代历史，找到最高分版本）
4. 基础版本（最高分版本）的 prompt

## 第三步：读取二郎神反馈
上一版本的扣分点和优化建议

## 第四步：基于基础版本 + 二郎神反馈生成新 prompt
- 基础版本：v{X}（当前最高分，分数：{score}分）
- 只能在基础版本上优化，不能继承低分版本的错误
...
### 第三步：读取图片
1. 参考图：`{project_path}/assets/`
2. 待评分图片：`{project_path}/slides/slide_XX_vN.png`

### 第四步：评分并输出结果
按照 SYSTEM_PROMPT_ERLANG.md 的格式输出评分报告
