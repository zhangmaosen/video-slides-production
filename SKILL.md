...
**适用场景**：
- 需要生成大量 prompt（10 个以上）
- API Key 不可用或不稳定
- 需要快速生成多个 prompt

**生成方式**：使用 OpenClaw 子代理批量生成

**优势**：
- ✅ **并行生成**：多个子代理同时运行，效率高
- ✅ **无需 API**：使用 OpenClaw 内置的 AI 能力
- ✅ **质量可控**：每个 prompt 都经过 AI 优化
- ✅ **自动保存**：生成结果自动保存到文件
...
for i in range(18):
    sessions_spawn(
        label=f"generate_slide_{i:02d}_prompt",
        mode="run",
        runTimeoutSeconds=60,
        task=f"""
你是一个顶级的幻灯片视觉设计师和 Qwen-Image 提示词专家。
请根据以下输入，生成一段可以直接用于 Qwen-Image 的、高质量的画面描述提示词。

**输入**：
- 视觉风格：混子说手绘 + 硬核工程拆解爆炸图风格，线条粗细变化，诙谐严肃混合，技术细节清晰，结构透视感强，舞台感构图
- 标题：xxx
- 观点：xxx
- 逐字稿：xxx
- 背景知识：xxx
...
**生成规则**：
1. 风格绝对统一，提取 1-2 个核心风格提示词放在句首
2. 标题必须用英文双引号包裹，放在画面合适位置，使用白色粗体字体，带有发光效果
3. 主体最详尽，描写材质、颜色、光影、动作姿态
4. 空间关系明确，说明左右、前后、上下的层级关系
5. 绝对正向描述，不使用负面词汇
6. 故事感核心，画面必须有冲突双方、动态动作、场景叙事
7. **所有生成的提示词必须使用中文！**
8. **只输出最终的 prompt 文本！不要输出任何解释、说明、标题、注释或其他内容。**

请生成 slide_{i:02d} 的图片提示词。
"""
    )
```

**生成时间**：
- 18 个 slides：约 5-10 分钟（16 个子代理并行）
- 每个 prompt：约 15-30 秒

**输出格式**：
- 文件：`prompts/slide_XX_final.txt`
- 内容：纯净的图片提示词文本（无额外说明）

**经验总结**（Tesla Semi 项目，2026-04-03）：
- ✅ 18 个 prompt 全部生成成功
- ✅ 平均生成时间：约 20 秒/个
- ✅ 质量良好：符合元模板要求
- ✅ 效率提升：相比逐个生成节省 80% 时间
